...
```csharp
    ...
    // First() ile FirstOrDefault() farkı : eğer aradığım kriterde veri yoksa FirsOrDefault null dönerken , First hata fırlatır
    try
    {
...
* SingleOrDefault()
```csharp
    // Tek bir veri veya null döneceğinden emin olduğumuz durumlarda SingleOrDefault'u kullanabiliriz. Aksi takdirde hata fırlatır
    try
    {
